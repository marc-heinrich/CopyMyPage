...
Meilenstein: [CopyMyPage dev0.0.19 (GitHub-Meilenstein 21)](https://github.com/marc-heinrich/CopyMyPage/milestone/21)
...
- Autoritative Implementierungsquelle ist ausschließlich `C:\wamp\www\joomla6`. Diese Datei beschreibt den dort zuletzt am 2026-09-02 geprüften Stand und trennt vorhandene Funktionen von Zielverträgen und offenen Freigabepunkten.
...
- GitHub-Meilenstein 21, Distributionspaket und Repository-Manifeste tragen verbindlich die Releasebezeichnung `0.0.19`. Die lokale Entwicklungsinstallation enthält historisch noch die Komponenten-/Systempluginmanifest- und Schemabezeichnung `0.0.20`; auch die Step-4-Deltadatei heißt weiterhin `0.0.20.sql`. Diese lokalen technischen Bezeichner bilden keinen separaten Entwicklungsmeilenstein. Die Basismigration `0.0.19.sql` enthält Warenkorb, Kundendaten und Sitzinventar; die historisch benannte Deltadatei ergänzt `payment_provider`, `terms_accepted_at`, `terms_snapshot` und den Index auf `booking_id`.
...
Alle fünf sichtbaren Schritte sind in der lokalen Joomla-Instanz implementiert. Schritt 4 baut beim GET eine autoritative, fail-closed Momentaufnahme auf; `orderreview.checkout` prüft Bedingungen, Zahlungsart, DPCalendar-Endpreis, Warenkorbrevision und Sitzzuordnungen erneut, erzeugt genau eine DPCalendar-Buchung samt Tickets und wandelt Warenkorb und Sitze um. Schritt 5 verwendet inzwischen statusgesicherte DPCalendar-`pay`-/`order`-Overrides: Eine Vollzugsmeldung hängt ausschließlich vom aktuellen DPCalendar-Buchungsstatus und der CopyMyPage-Datenintegrität ab, niemals vom bloßen Layoutaufruf. Ein einmaliger Zahlungs-Handoff, ein Callback-Guard und eine per Scheduler planbare Reconciliation für verlassene Zahlungsbuchungen sind vorhanden. Der echte PayPal-Sandbox-Rundlauf, produktive Schedulerbetrieb sowie E-Mail-, Ticket-, QR-Code- und Einlassprüfung bleiben offen. Die übrigen Parallel-, Negativ- und Backendkriterien bleiben davon unabhängige Produktivfreigabe-Gates.

| Bereich | Stand am 2026-09-02 | Nächste Grenze |
| --- | --- | --- |
| Schritt 1 – Ticketauswahl und Warenkorb | implementiert und regressionsgeprüft; fehlende Sitzzuordnung wird nun ebenfalls fail-closed behandelt | automatischen Navbar-Ablauf weiter manuell langzeitbeobachten; offene Parallel-/Negativfälle separat schließen |
| Schritt 2 – Sitzplatzwahl | sichtbarer Ablauf und Browserruntime einschließlich 178-Sitz-Stresstest abgeschlossen; Missing-Assignment-Grenze und öffentlicher DPCalendar-Buchungs-Guard sind umgesetzt und lokal geprüft | verbleibende Parallel-, Ablauf-, 200-Sitz-, Backend- und reine POST-Fallback-Gates schließen |
| Schritt 3 – Kundendaten | View, Route, Service, Controller, Joomla-Form und Warenkorbentwurf implementiert; Profiladresse → neuer Bestellentwurf als einmalige Projektion sowie die anschließende Trennung beider Datensätze browserseitig bestätigt | manuelle Kontoanlage mit Live-CAPTCHA und produktionsnahe Negativtests separat abnehmen |
| Schritt 4 – Prüfen und bestätigen | autoritativer GET und geschützter Checkout-POST sind implementiert; Teil-/Komplettlöschung im Drawer wurde mit zwei Events geprüft | sichtbare Orderreview nach einer Drawer-Mutation live invalidieren oder neu laden; schreibenden Commit, Rollback und Parallelfälle separat abnehmen; AGB redaktionell/juristisch freigeben |
| Schritt 5 – Zahlung und Abschluss | statusgesicherte `pay`-/`order`-Overrides, Ticket-/Sitzprojektion, einmaliger Zahlungs-Handoff, Callback-Guard und Scheduler-Reconciliation sind implementiert; Task-Registrierung und lokaler Leerlauf wurden geprüft | PayPal-Sandbox in der Produktivumgebung, Schedulerbetrieb, Status-/Abbruchfälle, E-Mail, Rechnung, Download, Ticket, QR-Code und Einlass vollständig abnehmen |
...
- Für Veranstaltungen mit fester Sitzwahl müssen alle Onlinebuchungen über den CopyMyPage-Ablauf erfolgen. Ein paralleler nativer DPCalendar-Buchungsweg ohne Sitzzuordnung ist für diese Events unzulässig. Das Systemplugin fängt deshalb direkte Site-Aufrufe der Buchungsformular-View sowie die öffentlichen Erzeugungs-/Speichertasks einschließlich `bookingform.add`, `bookingform.apply`, `bookingform.edit`, `bookingform.editAssociations`, `bookingform.save`, `bookingform.save2copy`, `bookingform.save2menu` und `bookingform.save2new` serverseitig ab und leitet bekannte CopyMyPage-Events zur Ticketauswahl um.
- Der DPCalendar-Schutz ist bewusst eng begrenzt: Zahlungs-, Rückkehr-, Stornierungs-, Ticket- und QR-Code-Aufrufe einer bereits durch CopyMyPage erzeugten Buchung bleiben erreichbar. Ein gesonderter Callback-Guard prüft bei verwalteten Buchungen Buchungsstatus, gestartete Transaktion, Anbieter und Zuordnung und führt einen unzulässigen Rücklauf ausschließlich zur autoritativen Statusansicht. Die interne Step-4-Übergabe verwendet DPCalendar-Modelle und den DPCalendar-Site-Controller direkt und benötigt keinen freigeschalteten öffentlichen Nebenweg.
...
DPCalendar-Buchung und -Tickets + umgewandelter CopyMyPage-Warenkorb
        ...
        └── kostenpflichtig: DPCalendar-Status 3 → layout=pay → Zahlungsplugin/Callback
                │
                ▼
Schritt 5 – statusgesicherter Abschluss / Tickets / QR-Code über CopyMyPage-Overrides
```
...
**Implementierte Sicherheitsgrenze:** `TicketReservationService` behandelt sowohl eine vollständig fehlende Event-Zuordnung als auch ein vorhandenes, aber nicht vollständig `ready`-fähiges Inventar fail-closed. Das Event bleibt dann mit `bookable = false`, `remaining = 0` und Status `unavailable` sichtbar, kann aber weder über Schritt 1 noch über eine direkte Mutation reserviert werden. Dieser Fall wurde am 2026-09-02 mit dem Katalogevent 8 erneut geprüft.

Die folgende DPCalendar-Mengenformel bleibt Bestandteil der gemeinsamen Kapazitätsberechnung, ist bei einem sitzpflichtigen Katalogevent aber niemals allein ausreichend. Ein allgemeiner `general_admission`-Modus gehört nicht zur ersten Ausbaustufe:
...
- Dauerhaft verlassene DPCalendar-Buchungen im zahlungsausstehenden Zustand `3` werden durch `PaymentReconciliationService` bearbeitet. Ohne gestartete Transaktion kann ein ausreichend alter, eindeutig verknüpfter und integrer Datensatz abgebrochen und freigegeben werden; bei vorhandener Transaktions-ID oder uneindeutigem Zustand wird niemals ein Providerergebnis geraten, sondern eine manuelle Prüfung gemeldet. Das Task-Plugin `plg_task_copymypage` stellt diesen Lauf planbar und mit Dry-run bereit.
...
- `TicketReservationService` bleibt Eigentümer der Mengen- und Kapazitätsreservierung. Für sitzpflichtige Katalogveranstaltungen verlangt er eine vorhandene und `ready`-fähige Zuordnung, fragt die zusätzliche effektive Kapazitätsgrenze beim Sitzinventar ab und stimmt abhängige Sitzhaltungen bei Mengenänderung, Entfernen und Leeren ab.
...
### Implementierte Service-Aufteilung für Schritt 5

- `BookingCompletionService` lädt eine CopyMyPage-verwaltete DPCalendar-Buchung autoritativ samt genau einem umgewandelten Warenkorb, Tickets und Sitzprojektion. Nur DPCalendar-Status `1` ergibt `completed = true`; Status `3`, `4`, `6`, `7`, `10` und unbekannte Werte erhalten eigene, nicht erfolgreiche Zustände. Widersprüche zwischen Buchung, Warenkorb, Tickets und Sitzen werden als Integritätsfehler ausgegeben.
- Die updatefesten DPCalendar-Overrides `booking/pay.php` und `booking/order.php` verwenden ausschließlich diesen Zustand. Der bloße Aufruf von `layout=order` kann deshalb bei einer verwalteten Buchung keine Vollzugsmeldung erzeugen. Nicht von CopyMyPage verwaltete Buchungen fallen in den vorgesehenen Zuständen auf das native DPCalendar-Layout zurück.
- `PaymentHandoffService` stellt einen 600 Sekunden gültigen, einmal verwendbaren und an die Joomla-Session gebundenen Handoff bereit. Das `pay`-Override startet den Anbieter nur nach gültigem POST-CSRF-Token, erfolgreichem Verbrauch dieses Handoffs und atomarem `beginPaymentAttempt()`.
- `PaymentReconciliationService` prüft alte umgewandelte Warenkörbe in begrenzten Batches. Eindeutig verlassene Status-`3`-Buchungen ohne Transaktion werden konsistent abgebrochen und freigegeben; terminale Zustände werden repariert, während gestartete oder unklare Providerfälle zur manuellen Prüfung gemeldet werden.
- Die zentrale DI-Registrierung liegt in `PaymentReconciliationServiceProvider`. System- und Task-Plugin verwenden denselben Provider; das Task-Plugin führt `copymypage.reconcile_payments` mit konfigurierbarem Timeout, Batchlimit und Dry-run aus.
...
- Warenkorbmutationen aus dem same-origin Drawer aktualisieren derzeit Warenkorbwurzeln und Navbar-Indikator, nicht jedoch eine bereits offene Orderreview-Projektion. Beim Test mit zwei Events blieb die sichtbare Übersicht nach Teilentfernung und anschließendem vollständigem Leeren bis zum Reload auf Revision `5`; ein neuer GET zeigte korrekt Revision `6` mit einem Event beziehungsweise nach dem Leeren den blockierten Leerzustand. Das ist ein offener UI-Invalidierungsfehler, keine serverseitige Integritätslücke: Der Checkout-POST verlangt weiterhin die erwartete Revision und vergleicht die neu berechnete Signatur mit `hash_equals()`.
...
6. [x] `TicketReservationService` berücksichtigt innerhalb der bestehenden Event-Sperre Inventarbereitschaft und effektive Onlinekapazität. Fehlende Zuordnung, `draft`, unvollständige Materialisierung und ungültige Kapazität sperren ein sitzpflichtiges Katalogevent fail-closed; der Fall ohne `#__copymypage_event_seating`-Zeile wurde mit Event 8 erneut geprüft.
...
8. [x] Die Step-4-Warenkorbumwandlung erzeugt eine DPCalendar-Buchung und deren Tickets, ordnet sie pro Event und Preisindex deterministisch den gehaltenen Sitzen zu und führt `held → booked` innerhalb derselben Transaktion aus. Die davon getrennte echte Zahlungs-, Produktiv- und Folgeprozessabnahme bleibt unten offen.
9. [x] Das CopyMyPage-Systemplugin schützt auf der Site alle geprüften öffentlichen DPCalendar-Einstiege, die eine neue Buchung für ein sitzpflichtiges CopyMyPage-Ticket-Event erzeugen könnten. Der Schutz gilt unabhängig davon, ob dessen Inventar noch `draft` oder bereits `ready` ist. Buchungsformular-View und Erzeugungs-/Speichertasks leiten zur Ticketauswahl; Administratorzugriffe, Zahlungsrückläufe und vorhandene Ticket-/Buchungsansichten bleiben erreichbar.
...
Der lokale Entwicklungsstand enthält `ready`-Testinventare, eine erreichbare Sitzwahl, den DPCalendar-Guard und den vollständigen Missing-Assignment-Fail-closed-Vertrag. Er ist wegen der weiterhin offenen Produktiv-, Parallel-, Zahlungs- und Folgeprozessprüfungen noch nicht als produktiv abgenommener Endzustand zu interpretieren. Vor Produktivbetrieb gilt weiterhin folgende Reihenfolge ohne ungeschütztes Übergangsfenster:
...
- Installations-, Update- und Deinstallations-SQL enthalten Warenkorb- und Sitztabellen. Ticketwarenkorb, Sitzschema, Checkout-/Audit-Erweiterung und Schritt 5 gehören gemeinsam zum GitHub-/Release-Meilenstein `dev0.0.19`. Die lokale Entwicklungsinstallation verwendet für das historische Step-4-Delta und ihren Joomla-Schemastand weiterhin die technische Bezeichnung `0.0.20`.
...
- Step-5-Status und Zahlungsübergabe: `components/com_copymypage/src/Service/BookingCompletionService.php`, `PaymentHandoffService.php`, `PaymentReconciliationService.php`, `templates/copymypage/html/com_dpcalendar/booking/pay.php`, `order.php` und `templates/copymypage/html/layouts/copymypage/tickets/completion.php`;
- Step-5-Reconciliation und gemeinsame DI: `components/com_copymypage/src/Service/PaymentReconciliationServiceProvider.php`, `plugins/task/copymypage/src/Extension/CopyMyPage.php` sowie Manifest, Serviceprovider und Sprachdateien des Task-Plugins;
...
Seit dem Audit vom 2026-09-02 zusätzlich abgeschlossen:

- vollständig fehlende Event-Sitzzuordnung wird in Landingpage, Schritt 1 und direkter Reservierung fail-closed behandelt;
- native öffentliche DPCalendar-Buchungserzeugung für CopyMyPage-Sitzveranstaltungen ist serverseitig gesperrt, während Zahlungsrückläufe und vorhandene Buchungen/Tickets erreichbar bleiben.

Vor Produktivfreigabe weiterhin zwingend zu erledigen:
...
- `CustomerDataService` verwendet die Profiladresse nur, wenn für den aktuellen Warenkorb noch kein Kundendatenentwurf existiert. Sobald ein Entwurf gespeichert wurde, bleibt er der autoritative Bestelldatensatz: Änderungen im Dashboard überschreiben ihn nicht und Änderungen im Ticketworkflow schreiben nicht in das Dashboardprofil zurück.
...
Der vollständige Browserlauf deckte Gast ohne Konto, das Öffnen und Validieren der optionalen Kontoerstellung, fehlerhafte und erfolgreiche Anmeldung, vollständige Profilvorbefüllung, Ergänzung einer fehlenden Rechnungsanschrift, Buttonzustände, Desktop-/Mobile-Anordnung und den geschützten Übergang zu Schritt 4 ab. Am 2026-09-02 wurde die Adressgrenze zusätzlich mit drei verschiedenen Anschriften geprüft: Profil A wurde in einen neuen Bestellentwurf vorbefüllt, das Speichern von Bestelladresse B ließ das Profil A unverändert, und die anschließende Dashboardänderung auf Profil C ließ den vorhandenen Bestellentwurf B unverändert. Die tatsächliche Anlage eines neuen Joomla-Kontos wurde wegen des live aktiven CAPTCHA nicht automatisiert abgeschlossen; dieser letzte interaktive Unterfall bleibt Teil des Benutzertests. Syntax-, Lint-, XML-, JSON-, Sprach-, Source-/Min- und CSS-`calc()`-Prüfungen waren erfolgreich. Alle angelegten Testbenutzer, Profil- und Datenschutzdatensätze, Testwarenkörbe, Sitzhaltungen und temporären Artefakte wurden anschließend entfernt.

### Schritt 4 – Prüfen und bestätigen

Status am 2026-09-02: Schritt 4 ist in der lokalen Joomla-Instanz als geschützte Bestellprüfung mit verbindlichem Abschluss-POST implementiert. Die vierte von fünf Schrittmarken ist aktiv. Der Einstieg aus Schritt 3 heißt weiterhin „Weiter zur Zusammenfassung“ und der bloße GET auf `view=orderreview` startet keine Zahlung. Erst „Zahlungspflichtig bestellen“ beziehungsweise „Kostenlos bestellen“ führt den transaktionalen Commit und die DPCalendar-Übergabe aus.
...
- Löschung, Stornierung, Erstattung oder Papierkorbstatus einer verknüpften DPCalendar-Buchung lösen über das CopyMyPage-Systemplugin die Freigabe der Sitze und den Warenkorbstatus `released` aus. Dauerhaft verlassene Status-`3`-Buchungen ohne gestartete Transaktion werden zusätzlich durch die Scheduler-Reconciliation behandelt; unklare oder bereits beim Anbieter gestartete Fälle bleiben bewusst manuell zu prüfen.

#### Warenkorbänderung während einer offenen Bestellprüfung

- Der Browserlauf vom 2026-09-02 begann mit zwei Events, Gesamtbetrag `26,00 €`, Warenkorbrevision `5` und gültiger Checkoutsignatur. Das Entfernen eines Events im Warenkorb-Drawer schrieb serverseitig Revision `6`; ein frischer Orderreview-GET zeigte korrekt nur noch ein Event und `12,00 €`.
- Die bereits offene Orderreview-Darstellung aktualisierte sich dabei nicht und zeigte weiterhin zwei Events, `26,00 €` und Revision `5`. Auch nach vollständigem Leeren blieb dieses alte DOM sichtbar, bis ein Reload den blockierten Leerzustand ausgab.
- Der Server übernimmt diesen veralteten Browserstand nicht: Der Checkout sperrt und lädt den Warenkorb erneut, prüft `expectedCartRevision` und berechnet die Checkoutsignatur neu. Offen ist eine clientseitige Invalidierung oder ein Reload der Orderreview, sobald der Drawer eine Warenkorbmutation meldet.
...
#### Native DPCalendar-Grundlage und CopyMyPage-Schutz
...
- Das native `order`-Layout zeigt unabhängig vom Zahlungsstatus „Danke für Ihre Buchung“. Für CopyMyPage-verwaltete Buchungen wird dieses unsichere Verhalten durch das installierte Override ersetzt; ein manuell angehängtes `layout=order` ist deshalb dort kein Erfolgsnachweis und erzeugt bei Status `3` keine Vollzugsmeldung.

#### Implementierter CopyMyPage-Abschlussvertrag

- `BookingCompletionService` lädt die DPCalendar-Buchung und verlangt für einen verwalteten Abschluss genau einen verknüpften CopyMyPage-Warenkorb. Abhängig vom Buchungsstatus werden zusätzlich Warenkorbumwandlung, Tickets und die vollständige Ticket-/Sitzprojektion geprüft. Ein Widerspruch wird sichtbar als Integritätsfehler behandelt und nie in einen Erfolg umgedeutet.
- Ausschließlich DPCalendar-Status `1` setzt `completed = true` und erlaubt Vollzugsmeldung sowie Ticket-, Beleg- und Rechnungsaktionen. Status `3` erscheint als ausstehend, `4` als Verarbeitung, `6` als storniert, `7` als erstattet und `10` als teilweise abgeschlossen; unbekannte Zustände bleiben ein nicht erfolgreicher Warnzustand. URL und Layoutname beeinflussen diese Entscheidung nicht.
- Die Overrides für `booking/pay.php` und `booking/order.php` verwenden dasselbe gemeinsame Abschlusslayout mit fünfter Schrittmarke, Buchungskennung, Veranstaltung, Ticketart und Sitzkennung. Nicht von CopyMyPage verwaltete DPCalendar-Buchungen fallen nur in den dafür vorgesehenen nativen Zuständen auf das Core-Layout zurück.
- Eine ausstehende Zahlung kann nur über einen 600 Sekunden gültigen, einmal verwendbaren und an die Sitzung gebundenen Handoff fortgesetzt werden. Das `pay`-Override fordert zusätzlich POST, Joomla-CSRF-Token, passenden Provider und den atomaren Startanspruch von `PaymentReconciliationService::beginPaymentAttempt()`; der bloße Layoutaufruf startet keinen Zahlungsanbieter.
- Der Callback-Guard lässt eine verwaltete Zahlungsrückkehr nur bei eindeutiger Warenkorb-/Buchungszuordnung, passendem Anbieter, zulässigem Status und nachweislich gestarteter Transaktion in den DPCalendar-Callback. Andernfalls führt er zur autoritativen Statusansicht, ohne Erfolg vorzutäuschen.
- `PaymentReconciliationService` verarbeitet ausreichend alte umgewandelte Warenkörbe in begrenzten Batches. Status `3` ohne Transaktions-ID kann konsistent abgebrochen und mitsamt Tickets/Sitzen freigegeben werden; terminale Status werden repariert. Status `3` mit Transaktions-ID, laufende Verarbeitung und unklare Zustände werden lediglich zur manuellen Prüfung gemeldet.
- Das Joomla-Task-Plugin stellt `copymypage.reconcile_payments` mit Timeout, Batchlimit und Dry-run bereit und bezieht den Service über die gemeinsame zentrale DI-Registrierung. Installation, Sichtbarkeit in der Aufgabenliste und ein lokaler Leerlauf ohne Fehler wurden geprüft.
- Noch offen bleibt der echte PayPal-Sandbox-Rundlauf einschließlich Redirect, Capture, Transaktions-ID, Rückkehr und DPCalendar-Status `1`. Ebenso sind produktiver Schedulerbetrieb, E-Mail, Rechnung/Beleg, Download, QR-Code und Einlassprüfung end-to-end abzunehmen.
...
- Für die Reconciliation ist lokal als Ausgangswert ein Alter von 60 Minuten, ein Schedulerintervall von 5 Minuten und ein Batchlimit von 50 vorgesehen. Diese Betriebswerte sowie die tatsächliche Cron-Ausführung müssen in der Produktivumgebung bestätigt werden.
- Statusabbildung, Handoff, Callback-Guard und Reconciliation sind technisch festgelegt. Noch zu definieren ist der betriebliche Prozess, mit dem Administratoren die gemeldeten unklaren oder beim Zahlungsanbieter bereits gestarteten Fälle prüfen und entscheiden.
- Der Abschluss ist als schmaler Template-Override des DPCalendar-`order`-/`pay`-Ablaufs mit gemeinsamer CopyMyPage-Statusprojektion umgesetzt. Diese Architekturentscheidung ist abgeschlossen.
- Zu entscheiden ist, ob und mit welcher expliziten Benutzeraktion eine bereits vorhandene Bestelladresse erneut aus dem später geänderten Dashboardprofil übernommen oder eine Bestelladresse ins Profil zurückgeschrieben werden soll. Eine automatische bidirektionale Synchronisierung findet derzeit bewusst nicht statt.
...
Die folgenden Kästchen sind Abnahmekriterien, keine bloße Liste vorhandener Klassen. Sie bleiben offen, bis der jeweilige End-to-End- beziehungsweise Negativtest nachvollziehbar ausgeführt wurde. Die Existenz des Codes allein gilt nicht als Abnahme; im Live-Baum wurde beim Audit keine automatisierte Step-2-Regressionssuite gefunden.
...
- [x] Fehlende Zuordnung, `draft`-Status, Teilmaterialisierung oder ungültige Inventarzustände machen ein sitzpflichtiges Event in Landingpage, Schritt 1 und direkten Mutationen nicht buchbar. Für Event 8 lieferte der Availability-Endpunkt ohne Zuordnung `bookable = false`, `remaining = 0` und `status = unavailable`. Eine reine Abweichung der DPCalendar-`capacity` wird weiterhin diagnostiziert und durch die kleinere gemeinsame Laufzeitgrenze sicher begrenzt.
- [x] Direkter Aufruf der öffentlichen Buchungsformular-View sowie die geprüften Tasks `bookingform.save` und `bookingform.save2new` konnten für CopyMyPage-Ticket-Events keine native Buchung ohne Sitzzuordnung erzeugen und wurden per HTTP 303 zur Ticketauswahl geführt. Der Guard deckt zusätzlich die übrigen bekannten Erzeugungs-/Speicheraliase ab; die Tests erzeugten keine DPCalendar-Buchung.
- [ ] Erlaubte DPCalendar-Zahlungsrückläufe sowie bestehende Buchungs-, Ticket- und QR-Code-Ansichten sind im Code bewusst vom Erzeugungs-Guard getrennt. Ihr vollständiger Nachweis bleibt Teil des produktiven PayPal-/Ticket-End-to-End-Laufs.
...
- [x] Evententfernung und vollständiges Leeren geben alle zugehörigen gehaltenen Sitze innerhalb derselben Transaktion frei. Dies wurde am 2026-09-02 im Zwei-Event-Browserlauf sowohl für eine Teilentfernung als auch für das anschließende Leeren bestätigt.
...
- [x] Die Adressschichten wurden am 2026-09-02 gezielt gegeneinander geprüft: Profil A befüllte einen neuen Bestellentwurf, Bestelladresse B änderte Profil A nicht, und die spätere Dashboardadresse C änderte den bestehenden Bestellentwurf B nicht. Testkonto, Profiladresse, Bestellentwurf, Warenkorb und Sitzhaltung wurden danach vollständig entfernt.
...
### Verifizierter Step-4-Stand und offene Abnahme vom 2026-09-02
...
- [x] Im Zwei-Event-Lauf wurden Teilentfernung und vollständiges Leeren über den Warenkorb-Drawer geprüft. Ein frischer Orderreview-GET zeigte nach jeder Mutation den richtigen autoritativen Zustand; der Lauf erzeugte weder DPCalendar-Buchung noch Zahlung.
- [ ] Eine bereits offene Orderreview aktualisiert sich nach der Drawer-Mutation derzeit nicht. Bis zum Reload bleiben alte Events, Summe, Revision und Signatur sichtbar; die serverseitige Revisions- und Signaturprüfung verhindert zwar einen veralteten Commit, die sichtbare Ansicht muss aber noch invalidiert oder neu geladen werden.
...
- [x] Step-5-Zustandsansicht, Schutz vor direkt erzwungenem `layout=order`, einmaliger Zahlungs-Handoff, Callback-Guard und Timeout/Reconciliation für verlassene Status-`3`-Buchungen sind implementiert und im Live-Code revidiert. Eine Vollzugsmeldung ist ausschließlich bei autoritativem DPCalendar-Status `1` möglich.
- [ ] E-Mail-, Rechnungs-, Ticket-, Download-, QR-Code- und Einlassprüfung bleiben zusammen mit dem echten PayPal-Lauf offen.

### Verifizierter Step-5-Strukturstand und offene Produktivabnahme vom 2026-09-02

- [x] `BookingCompletionService` unterscheidet DPCalendar-Status `1`, `3`, `4`, `6`, `7`, `10` und unbekannte Werte; nur Status `1` ist `completed`. Verknüpfter Warenkorb, Tickets und Sitzprojektion werden abhängig vom Zustand auf Integrität geprüft.
- [x] Die DPCalendar-Overrides für `pay` und `order` verwenden dieselbe Statusprojektion. Der Aufruf von `layout=order` allein kann für eine CopyMyPage-verwaltete Buchung keine Vollzugsmeldung erzeugen.
- [x] Der Zahlungsstart erfordert gültigen POST-CSRF-Schutz, einen einmal verwendbaren sitzungsgebundenen Handoff und einen atomaren Startanspruch. Der Callback-Guard verlangt passende Buchung, Warenkorb, Provider, Status und gestartete Transaktion.
- [x] `PaymentReconciliationService` gibt nur eindeutig verlassene Buchungen ohne gestartete Transaktion automatisch frei und meldet unklare beziehungsweise gestartete Fälle zur manuellen Prüfung. System- und Task-Plugin beziehen ihn über denselben zentralen DI-Provider.
- [x] Das Task-Plugin ist in der lokalen Testinstallation sichtbar und ließ sich ohne Fehlermeldung ausführen. Sprachdateien und Paketzuordnung wurden auf den Distributionsstand `0.0.19` gebracht.
- [ ] Die Zustände müssen noch mit kontrollierten, vollständig verknüpften Testbuchungen end-to-end durchlaufen werden; insbesondere verspätete oder wiederholte Rückläufe dürfen eine bereits freigegebene Sitzzuordnung nicht reaktivieren.
- [ ] PayPal-Sandbox einschließlich Redirect, Orderanlage, Freigabe, Capture, Transaktions-ID, Callback, Status `1` und Vollzugsmeldung wird wie vereinbart in der Produktivumgebung geprüft.
- [ ] Der Schedulerbetrieb mit vorgesehenen Werten 60 Minuten / 5 Minuten / Batch 50 sowie Benachrichtigung und Bearbeitung manueller Prüffälle muss produktiv überwacht werden.
- [ ] E-Mail, Rechnung/Beleg, Ticketdownload, Sitzbeschriftung, QR-Code und Einlassprüfung sind noch end-to-end abzunehmen.
...
- 2026-09-02: Schritt 5 für `dev0.0.19` mit statusgesicherten DPCalendar-`pay`-/`order`-Overrides, gemeinsamer Buchungs-/Ticket-/Sitzprojektion, einmaligem Zahlungs-Handoff, Callback-Guard und fehlertoleranter Reconciliation dokumentiert. Das neue Task-Plugin verwendet die zentrale DI-Registrierung; Installation, Aufgabenlisteneintrag, Sprachdateien und lokaler Leerlauf wurden geprüft.
- 2026-09-02: Missing-Assignment-Fail-closed und öffentlicher DPCalendar-Buchungs-Guard im Live-System revidiert und geprüft. Der Browser-Stresstest bestätigte Teil-/Komplettlöschung und serverseitige Orderreview-Neuprojektion, deckte jedoch die noch fehlende Live-Invalidierung einer bereits offenen Orderreview auf. Die einmalige Profil-zu-Bestellentwurf-Projektion wurde mit drei Adressständen bestätigt; Testdaten und temporäre Artefakte wurden entfernt. Der echte PayPal-Rundlauf bleibt der Produktivabnahme vorbehalten.
